...
class HelpDeskReuestFormWidget extends ConsumerWidget {
  final _VSControllerParams params;

  // final _ViewState state;
  // final _VSController stateController;
  const HelpDeskReuestFormWidget({
    super.key,
    // required this.state,
        ...
        child: Form(
          ...
          child: Column(
            children: [
              ...
              KDropdownField<HelpDeskSubCategoryItem>(
                value: null,
                fieldHeadingText: 'Sub Category Type *',
                hintText: 'Select service Type',

                items:
                    state.subCategories
                        ?.map<KDropdownItem<HelpDeskSubCategoryItem>>(
                          (opt) => KDropdownItem<HelpDeskSubCategoryItem>(
                            value: opt,
                            child: Text(opt.name ?? ''),
                          ),
                        )
                ...
                onChanged: (v) {
                  if (v != null) {
                    stateController.onSelectSubCategory(v);
                  }
                },
              ),
              ...
              KDropdownField<HelpDeskIssueTypeItem>(
                value: null,
                fieldHeadingText: 'Issue Type *',
                hintText: 'Select issue type',

                items: state.issueTypes
                    .map<KDropdownItem<HelpDeskIssueTypeItem>>(
                      (opt) => KDropdownItem<HelpDeskIssueTypeItem>(
                        value: opt,
                        child: Text(opt.name ?? ''),
                      ),
                    )
                ...
                onChanged: (v) {
                  if (v != null) {
                    stateController.onSelectIssueType(v);
                  }
                },
              ),
              Padding(
                padding: EdgeInsets.only(bottom: 15.toAutoScaledHeight),
                child: KTextField(
                  fieldHeadingText: 'Extension Number (Optional)',
                  hintText: 'Enter here...',
                  controller: stateController.extensionNumberController,
                ),
              ),
              ...
              Padding(
                ...
                child: KTextField(
                  ...
                  hintText: 'Enter here...',
                  controller: stateController.problemController,
                  validator: (value) => (value == null || value.isEmpty
...
class HelpDeskRequestHeader extends StatelessWidget {
  final String title;
  final String subtitle;
  final VoidCallback? onClose;

  const HelpDeskRequestHeader({
    super.key,
