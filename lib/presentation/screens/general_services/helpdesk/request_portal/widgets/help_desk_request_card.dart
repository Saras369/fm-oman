...
class HelpDeskRequestCard extends ConsumerStatefulWidget {
  final List<HelpDeskRequestItem> myRequests;
  final List<HelpDeskRequestItem> actionItems;
  final _ViewState state;
  final _VSController stateController;
  final _VSControllerParams params;

  const HelpDeskRequestCard({
    super.key,
  ...
  @override
  ConsumerState<HelpDeskRequestCard> createState() =>
      _HelpDeskRequestCardState();
}

class _HelpDeskRequestCardState extends ConsumerState<HelpDeskRequestCard> {
  int selectedTab = 0; // 0=My Requests, 1=Action Items
  int currentPage = 1;

  List<HelpDeskRequestItem> get currentItems {
              ...
              children: [
                ...
                Expanded(
                  child: ListView.separated(
                    ...
                    itemBuilder: (_, index) => _HelpDeskRequestCardItem(
                      request: items[index],
...
class _HelpDeskRequestCardItem extends StatelessWidget {
  final HelpDeskRequestItem request;
  final _VSController stateController;

  const _HelpDeskRequestCardItem({
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            _buildRow('Request ID:', '${request.id?.id ?? 0}', textTheme),
            ...
            Row(
              children: [
                Expanded(
                  child: _buildRow(
                    'Date:',
                    DateTime.parse(request.requestDate ?? '').formattedDate,
                    textTheme,
                  ),
                ),
            ...
            Row(
              children: [
                ...
                IconButton(
                  ...
                  onPressed: () {
                    stateController.onPressRequestDetails(request.id?.id ?? 0);
                  },
                ),
              ],
            ),
