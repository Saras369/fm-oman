...
class _VSController extends StateNotifier<_ViewState> {
  ...
  void onPressRequestMenuActionItem(
    ...
    KAppX.extendedRouter.dialog.showKDialog(
      ...
      builder: (context) {
        return StationeryRequestForm(params: params);
      },
    );
  }
  ...
  final stationeryRepo = StationeryRepo();

  Future<void> fetchApproverKPIStats() async {
    try {
      final statsData = await stationeryRepo.fetchApproverKPIStats();
  ...
  Future<void> fetchKPIStats() async {
    try {
      final statsData = await stationeryRepo.fetchMyKPIStats();
  ...
  Future<void> fetchStatusBreakDownData() async {
    try {
      final statusBrekdownData = await stationeryRepo.fetchStatusBreakdown({
  ...
  Future<void> fetchMonthlyTrendData() async {
    try {
      final trendData = await stationeryRepo.fetchMonthlyTrend({
        'year': 2025,
      }, true);
  ...
  Future<void> fetchMaterials() async {
    try {
      final materials = await stationeryRepo.fetchStationeryMaterialItem();
  ...
  Future<void> fetchOfficesList() async {
    try {
      final offices = await stationeryRepo.fetchStationeryOfficeItem();

      if (offices != null) {
        state = state.copyWith(officeList: offices);
      }
    } on ApiException catch (apiError) {
      Fluttertoast.showToast(msg: apiError.message, timeInSecForIosWeb: 3);
    } catch (e) {}
  ...
  void onSelectOffice(StationeryOfficeItem office) {
    state = state.copyWith(selectedOffice: office);
  }

  void onSelectStationeryMaterial(StationeryMaterialItem material) {
    state = state.copyWith(selectedMaterial: material);
  }

  Future<void> fetchMyRequests() async {
    try {
      final myRequests = await stationeryRepo.fetchStationeryMyRequests();
