...
class _VSController extends StateNotifier<_ViewState> {
  ...
  void onPressRequestMenuActionItem() {
    KAppX.extendedRouter.dialog.showKDialog(
      ...
      builder: (context) {
        return ParcelServicesRequsetForm(params: params);
      },
    );
  }
  ...
  final parcelServicesRepo = ParcelServicesRepo();

  Future<void> fetchApproverKPIStats() async {
    try {
      final statsData = await parcelServicesRepo.fetchApproverKPIStats();
  ...
  Future<void> fetchKPIStats() async {
    try {
      final statsData = await parcelServicesRepo.fetchMyKPIStats();
  ...
  Future<void> fetchStatusBreakDownData() async {
    try {
      final statusBrekdownData = await parcelServicesRepo.fetchStatusBreakdown({
  ...
  Future<void> fetchMonthlyTrendData() async {
    try {
      final trendData = await parcelServicesRepo.fetchMonthlyTrend({
        'year': 2025,
      }, true);
